<template>
  <div class="login-container">
    <el-form class="login-form" autoComplete="on" :model="loginForm" :rules="loginRules" ref="loginForm" label-position="left">
      ...
      <el-form-item prop="password" :error="errorInfo">
    ...
    handleLogin() {
      this.$refs.loginForm.validate(valid => {
        if (valid) {
          ...
          // var url = 'http://localhost:3412' + '/login';
          this.$http.post(url, {username: this.loginForm.username, password: this.loginForm.password}).then(response => {
            var s = response.headers.authorization.split(' ');
            var token = s[1];
            setToken(token);
            this.loading = false;
